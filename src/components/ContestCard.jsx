...
const ContestCard = ({
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      setOngoingContestTimeLeft(getTimeLeft(contest.end));
    }, 1000);
  ...
  return (
    <div className="border border-gray-300 rounded-xl p-4">
      ...
      {isOngoingContest && (
        <div>
          <div className="bg-red-600 text-white animate-pulse w-fit rounded-xl px-4 py-2 ">
            Live
          </div>
          <div className="mt-2 bg-linear-to-r from-red-600 via-gray-900 to-red-600 text-white w-fit rounded-xl px-4 py-2 flex space-x-2">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 -960 960 960"
            ...
            <span>{ongoingContestTimeLeft.hours} h : </span>
            <span>{ongoingContestTimeLeft.minutes} min : </span>
            <span className="mr-1">{ongoingContestTimeLeft.seconds} sec </span>
            Left
          </div>
        </div>
      )}
